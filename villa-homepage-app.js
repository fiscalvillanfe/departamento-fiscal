...
async function processFiles(fileList) {
  ...
  // === ALTERADO: gera sub-ZIPs por empresa, com XMLs soltos dentro de cada sub-ZIP
  async function buildNonZip() {
    // 1) Coletar somente as notas fora do estado por empresa
    const nonByCompany = {};
    ...
    // 3) Criar um sub-ZIP por empresa (conteúdo: apenas os XMLs na raiz)
    for (const [company, items] of Object.entries(nonByCompany)) {
      const sub = new JSZip();
      for (const rec of items) {
        sub.file(rec.filename, rec.text); // sem subpasta
      }
      // usar uint8array para garantir detecção correta do arquivo .zip dentro do zip
    ...
    // 5) ZIP final (blob para download)
    return await zip.generateAsync({ type:'blob' });
  }
